...
Small, reproducible evidence is committed under `artifacts/metrics/` and `artifacts/figures/`, including experiment tables, per-label metrics, calibration data, prediction exports, TF-IDF feature weights and misclassified examples. Trained model files and transformer checkpoints are intentionally ignored because they are regenerated by the commands above.
